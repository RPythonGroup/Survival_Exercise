...
## Hand-on 실습준비
실제데이터를 가지고 단계별로 진행되는 과정을 따라 할 수 있도록 R로 작성된 Quarto 프로젝트를 만들었습니다.
이 Quarto 프로젝트 파일들은 연구회의 Github 원격저장소에 저장해 두었습니다.
따라서 실습을 위해서는 로컬인 자신의 PC로 원격저장소의 프로젝트 파일들은 cloning(=다운로드)하셔야 합니다. 
Cloning을 위해서는 RStudio만 있는 것이 아니라 Git도 설치되어 있어야 합니다. 혹시라도 나중에 합류한 회원분들 중 Git 설치가 안되어 있으면 다음을 참고하여 설치해 두시기 바랍니다.
https://rpythongroup.github.io/Website/posts/tools/Git/Git_setup.html


### git clone
Git을 이용한 clone은 마치 새로운 프로젝트를 만드는 것과 같습니다. 그래서 Rstudio에서 모든 프로젝트들은 닫은 상태에서 진행하시는 것이 바람직합니다. 윈도우 운영체제에서는 RStudio에서 모든 프로젝트를 다 닫는다면 default working directory상에 RStudio가 열려있게 됩니다. 연구회에서는 default working directory를 `C:\Projects\`로 미리 지정하는 것으로 추천드렸습니다.
...
### 생존곡선 만들기
물론 생존곡선을 그리는 방법은 많습니다. 여기서는 일단 가장 간단한 형태를 먼저 그려 보겠습니다. 
plot() 함수는 제네릭 함수로, 기본적으로 산점도를 그리지만, Surv 객체를 인자로 받으면 내부적으로 plot.survfit() method를 호출하여 Kaplan-Meier(KM) 곡선을 반환합니다.
